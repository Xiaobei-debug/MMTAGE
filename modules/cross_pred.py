...
from modules.vae_model import TransformerDecoder
class EmbToTCRDecoder_Trans(nn.Module):
    def __init__(self, tcr_len=26, vocab_size=24):
        super(EmbToTCRDecoder_Trans, self).__init__()
        self.dec = TransformerDecoder(256)
        self.relu = nn.ReLU()
        self.tcr_len = tcr_len
        self.vocab_size = vocab_size

    def forward(self, emb, TCR_index):
        pred = self.dec(emb, TCR_index)
        cross_loss = F.cross_entropy(
            pred.reshape(-1, 24),
            TCR_index[:, 1:].reshape(-1),
        )
        return cross_loss
